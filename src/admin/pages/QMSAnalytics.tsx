    ...
    <div className="flex justify-center items-center bg-cover bg-center bg-main-building">
      <div className="text-gray-900 bg-gray-200">
        <div className="p-4 flex">
          <h1 className="text-2xl">Quality Management System Evaluation</h1>
        </div>
        <div className="px-3 py-4 flex justify-center">
          
          <button
            id="IADropdownDefaultButton"
          ...
          {dropdownOpen && (
            ...
            >
              <ul
                ...
                aria-labelledby="IADropdownDefaultButton"
              >
                <li>
                  <a
                    href="#"
            ...
            <thead className="border-b">
              <tr>
                ...
                {questions.map((question, index) => (
                  <th key={index} className="text-left p-3 px-5">{question}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              {responses.map((response, index) => (
                <tr key={index} className="border-b hover:bg-orange-100 bg-gray-100">
                  {response.map((value, idx) => (
                    <td key={idx} className="p-3 px-5">{value}</td>
                  ))}
                </tr>
              ))}
            </tbody>
